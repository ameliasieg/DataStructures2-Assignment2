...
namespace cop4530 {
    ...
    class List {
       ...
       public:
          ...
          {
              // TO BE FILLED - DONE
              init(); // Make a list - initialize it
              while(start != end)
              {
          ...
          List (std::initializer_list<T> iList)
          {
              ...
              {
                push_back(*itr); // Dereferencing the pointer, getting the actual data inside
              }
          }
          ...
          List& operator= (std::initializer_list<T> iList)
          {
              // TO BE FILLED - DONE
              List newList(iList); 
              *this = newList;       //Update calling object
          ...
          void reverse()
          {
              ...
              if(!empty())
              {
                ...
                while(itr != nullptr) // While the current iterator exists...
                {
                    std::swap(itr->next, itr->prev);
                    itr = itr->prev; // Previous becomes next
                }
              }
